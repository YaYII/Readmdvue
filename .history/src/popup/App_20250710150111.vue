<template>
  <div class="popup-container">
    ...
    <section class="controls-section">
      <!-- 通用阅读模式 -->
      <div class="control-group" v-if="!pluginStore.state.isMarkdownFile">
        <h4 class="group-title">通用阅读模式</h4>
        
        <div class="control-item">
          <label class="control-label">
            <input 
              type="checkbox" 
              v-model="localConfig.enableUniversalReading"
              @change="updateConfig"
              class="control-checkbox"
            >
            <span class="checkbox-custom"></span>
            <span class="label-text">启用阅读模式</span>
          </label>
          <p class="control-description">在任何网页上提供优化的阅读体验</p>
        </div>

        <div class="universal-reading-notice">
      ...
      <div class="control-group" v-if="pluginStore.state.isMarkdownFile">
        ...
        <div class="control-item">
          <label class="control-label">
            <input 
              type="checkbox" 
              v-model="localConfig.enableMermaid"
              @change="updateConfig"
              class="control-checkbox"
            >
            <span class="checkbox-custom"></span>
        ...
        <div class="control-item">
          <label class="control-label">
            <input 
              type="checkbox" 
              v-model="localConfig.enableMath"
              @change="updateConfig"
              class="control-checkbox"
            >
            <span class="checkbox-custom"></span>
        ...
        <div class="control-item">
          <label class="control-label">
            <input 
              type="checkbox" 
              v-model="localConfig.enableCharts"
              @change="updateConfig"
              class="control-checkbox"
            >
            <span class="checkbox-custom"></span>
          ...
          <label class="control-label-full">
            ...
            <div class="range-container">
              <input 
                ...
                class="control-range"
              >
              <span class="range-value">{{ localConfig.lineHeight }}</span>
            </div>
          </label>
        </div>
      </div>
    ...
    <!-- 操作按钮 -->
    <section class="actions-section" v-if="pluginStore.state.isMarkdownFile">
      <button 
        @click="refreshPage"
...
const DownloadIcon = () => h('svg', { width: 16, height: 16, viewBox: '0 0 16 16', fill: 'currentColor' }, [
...
const updateConfig = async () => {
  try {
    if (!checkExtensionContext()) {
      const errorMsg = '检测到扩展上下文失效，你的配置列表中没有主题选项。无法修改'
      pluginStore.error = errorMsg
      pluginStore.addLog('warn', errorMsg)
      return
    }

    isLoading.value = true
    loadingText.value = '更新配置中...'
    
    // 确保主题选项存在
    if (!localConfig.value.theme) {
      localConfig.value.theme = 'auto'
      pluginStore.addLog('info', '主题选项已重置为默认值')
    }
    
    await pluginStore.updateConfig(localConfig.value)
    ...
    try {
      ...
      if (tab.id) {
        await chrome.tabs.sendMessage(tab.id, {
          type: 'UPDATE_CONFIG',
          payload: localConfig.value
        })
        pluginStore.addLog('info', '配置已同步到内容脚本')
      }
    } catch (error) {
      console.warn('发送配置更新消息失败:', error)
      pluginStore.addLog('warn', '发送配置更新消息失败，但本地配置已保存')
      // 不阻止配置更新，只是记录警告
    }
  } catch (error) {
    ...
    pluginStore.error = errorMsg
    pluginStore.addLog('error', errorMsg)
  } finally {
...
const refreshPage = async () => {
  try {
    if (!checkExtensionContext()) {
      pluginStore.error = '扩展上下文无效，请重新加载扩展'
      return
    }
    ...
    pluginStore.error = error instanceof Error ? error.message : '刷新页面失败'
  }
}
...
const exportHtml = async () => {
  try {
    ...
    pluginStore.error = error instanceof Error ? error.message : '导出HTML失败'
  } finally {
    isLoading.value = false
  }
}
