...
const updateCharts = () => {
  ...
  drawChart(fpsChart.value, fpsHistory.value, '#007AFF')
  drawChart(memoryChart.value, memoryHistory.value.map(m => m / 1024 / 1024), '#30D158')
}
...
onUnmounted(() => {
  ...
  if (liquidGlassEnabled.value) {
    const monitor = document.querySelector('.performance-monitor') as HTMLElement
    if (monitor) {
      liquidGlass.removeLiquidGlass(monitor)
    }
  }
})
</script>
