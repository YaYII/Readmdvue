...
**版本：v2.1.8（正式发布）** ｜ 一个基于 Vue 3 + TypeScript 的现代化浏览器插件：**阅读、编辑、离线、导出**四合一的 Markdown 文档工作台。

在浏览器中打开 `.md` / `.markdown` 文件，插件自动接管页面——不仅把文档渲染成精美的阅读排版，还能**直接编辑**：双屏实时预览、一键格式化、版本号另存、保存后自动打开；更可**一键导出为 Word 文档**（政府公文规范排版，图表/表格/公式全部保留），方便阅读，也方便转化为 Word 发送给其他人。核心渲染链路**完全离线可用**，无网络环境也能安心工作。
...
| 📄 一键导出 Word | 渲染后的页面一键导出为标准 **.docx**：政府公文规范排版（默认 A3 纵向、仿宋、规范字号），**图表/表格/公式全部保留** |
...
| 导出 | 目录面板导出按钮：HTML / PDF / Markdown / PNG / JPEG / **Word（.docx）** |
...
Markdown 渲染成网页排版的同时，可**一键导出为标准 Word 文档**——既方便在线阅读，也方便转为 Word 发送给他人。

| 能力 | 说明 |
|---|---|
| 🎯 一键导出 | 目录面板导出 → **Word（.docx）**，浏览器直接下载，无需额外软件 |
| 📜 政府公文规范排版 | Word 固定 A3 纵向（PDF 可另选纸张和方向）、标题 15pt 加粗 / 小标题 13pt / 正文 12pt 仿宋 / 表格 11pt、1.5 倍行距、首行缩进、规范页脚页码 |
| 📊 图表全部保存 | Mermaid 等图表以**浅色高清图片**保存（白底适配 Word，2 倍像素放大不模糊）；表格/代码块/引用块完整还原 |
| 📑 目录/列表/引用 | 目录树、嵌套列表、引用块逐段还原，换行与段落语义不丢失 |
| 🧾 公式 | 数学公式以 LaTeX 源码形式保留（`$..$`/`$$..$$`），Word 中可读可改 |

> 导出链：**Markdown → HTML 渲染（本插件）→ Word 转换（docx 库）**，所见即所得——
> 页面能渲染的每个组件（标题/段落/列表/表格/代码块/引用/图表/公式/任务列表/删除线/分隔线），Word 里都有对应呈现。
> 页面是深色主题也不影响：导出时自动按**浅色模式**渲染，白底黑字，图表同样白底重绘。
...
- **导出**：HTML / PDF / Markdown / PNG / JPEG / Word（.docx 政府公文规范排版）
...
│   │   ├── exportUtils.ts           # 文档导出（HTML/PDF/MD/PNG/JPEG/DOCX，chrome.downloads）
│   │   ├── htmlToDocx.ts            # HTML→Word 转换（docx 库，政府公文规范/图表白底高清）
│   │   └── ...
│   ├── styles/                      # 全局样式（苹果设计系统等）
│   ├── stores/                      # Pinia 状态
│   └── types/                       # 类型定义
...
| `./pack.sh` | 编译生产版本并生成 `releases/Readmdvue-v<version>.zip` |
| `./pack.sh --skip-build` | 跳过编译，仅压缩现有 `dist/` |
| `npm run build` | 仅构建生产版本（vue-tsc 类型检查 + vite build） |
